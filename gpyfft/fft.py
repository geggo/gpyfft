...
class FFT(object):
    def __init__(self, context, queue, in_array, out_array=None, axes = None,
        ...
        if (in_kind == 'f' or out_kind == 'f') and not real:
            real = True
            warnings.warn("Setting 'real' to True because input or output is real (specify real=True to avoid this warning)")
        ...
        if real:
            if in_kind == 'f':
                ...
                if t_inplace:
                    ...
                    # array.
                    curstride = in_array.strides[real_axis]
                    ...
                    assert  expectedstride < in_array.strides[real_axis], "Not enough padding in array for real-to-complex in axis %d (found stride of %d bytes (%d elements), expected %d bytes (%d elements)" % (real_axis, curstride, curstride_elems, expectedstride, expectedstride_elems)
                else:
                ...
                if fft_shape is not None:
                    assert fft_shape == in_array.shape, "fft_shape set incorrectly (not necessary to send fft_shape for real-to-complex)"
            else:
                # complex-to-real
                real_axis = axes_transform[0]
                if t_inplace:
                    ...
                    assert in_array.shape == tuple(expected_in_shape), \
                        'input array shape %s does not match expected shape: %s'%(in_array.shape, expected_in_shape)
                    if fft_shape is not None:
                        assert fft_shape == expected_in_shape, "fft_shape set incorrectly (not necessary to send fft_shape for out-of-place complex-to-real)"
